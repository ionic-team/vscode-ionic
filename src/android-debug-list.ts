...
export async function getAndroidWebViewList(hasCapacitorAndroid: boolean): Promise<Recommendation[]> {
	...
	for (const device of devices) {
		...
		for (const webview of webviews) {
			const r = new Recommendation(`Click to debug ${webview.packageName} ${webview.versionName}`, `(${device.product})`, `${webview.packageName}`, vscode.TreeItemCollapsibleState.None, getCommand(), undefined);
			r.setIcon('debug');
		...
		if (webviews.length == 0) {
			const r = new Recommendation('test', 'No Web View', device.product, vscode.TreeItemCollapsibleState.None, getCommand(), undefined);
			r.setIcon('android');
			result.push(r);
		}
	}
