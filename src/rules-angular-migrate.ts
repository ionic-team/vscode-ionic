...
export function angularMigrate(latestVersion: string): Tip {
	...
	const tip = new Tip(
		...
		TipType.Run, undefined,
		'ng update @angular/cli @angular/core --allow-dirty --force', `Upgrade to v${latest.major}`, undefined, 'https://angular.io/cli/update').showProgressDialog();		
	...
	if (next == latest.major) {
		...
		tip.setSecondCommand(
			...
			`ng update @angular/cli@${current.major} @angular/core@${current.major} --allow-dirty --force`
		);		
