...
export function stripJsonComments(jsonString: string, { whitespace = true, trailingCommas = false } = {}) {
  ...
  return (
    result +
    buffer +
    (isInsideComment ? strip(jsonString.slice(offset), undefined as any, undefined as any) : jsonString.slice(offset))
  );
}
